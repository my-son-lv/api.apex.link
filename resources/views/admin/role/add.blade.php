    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{route('admin.role.add')}}" class="form" method="post" id="dataForm">
                        <input type="hidden" value="{{csrf_token()}}" name="_token">
                        <div class="form-group row">
                            <label for="example-text-input" class="col-2 col-form-label text-right">名称：</label>
                            <div class="col-8">
                                <input type="text" name="name" placeholder="角色名称" class="form-control" id="name">
                            </div>
                        </div>
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            $.post({url:"{{route('admin.menu.menuList.white')}}",type:"POST",async:false,dataType:'json',data:{'_token' : "{{csrf_token()}}"},success:function(res){zNodes = res;}});
