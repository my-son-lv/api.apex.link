...
class CreateMembersInfoCheckedTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('members_info_checked', function (Blueprint $table) {
            $table->increments('id');
            ...
            $table->tinyInteger('status')->default(0)->comment('0新用户  1提交中 2审核通过 3审核驳回');
            $table->tinyInteger('check_log_id')->nullable()->comment('审核日志ID');
            $table->tinyInteger('pay_type')->default(1)->comment('薪资类型  1 10k-13k 2 13k-16k 3 16k-20k  4 20k-25k   5  >25k');
    ...
    {
        Schema::dropIfExists('members_info_checked');
    }
}
